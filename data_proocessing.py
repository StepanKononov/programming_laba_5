...
def prediction(data_Y, start_index, step, data_X=None):
    result_Y = data_Y.copy()
    if data_X is None:
        data_X= [i for i in range(len(data_Y))]
    for i in range(start_index, len(data_Y), step):
        a, b = mnk([data_X[i - 1], data_X[i - 2]], [data_Y[i - 1], data_Y[i - 2]])
        r_y = a * data_X[i] + b
        result_Y[i] = r_y
    return result_Y
...
def smoothing(points, deviation=98):
    deviation /= 100
    smoothed_points = []
    for point in points:
        try:
...
def deviation (ar1, ar2):
    d = 0
    for i in range(len(ar1)):
        d += abs(abs(ar1[i]) -  abs(ar2[i]))
    return d